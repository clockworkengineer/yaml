...
mod tests {
    ...
    fn test_simple_sequence() {
        ...
        let result = parse_sequence_with_tokens(&mut stream, 0, 0, &directives, &ctx, 0).unwrap();
    ...
    fn test_empty_sequence() {
        ...
        if let Node::Array(items) = result {
            ...
            assert!(matches!(items[0], Node::None));
    ...
    #[test]
    fn test_sequence_with_decorators() {
        let yaml = b"- !!str item1\n- &anchor item2";
        let mut source = Buffer::new(yaml);
        let directives = DirectiveContext::new();
        let ctx = ParsingContext::default();
        let mut stream = TokenStream::new(&mut source, &directives, false).unwrap();
        ...
        if let Node::Array(items) = result {
            assert_eq!(items.len(), 2);
            assert!(matches!(items[0], Node::Str(_, _, _)));
            assert!(matches!(items[1], Node::Anchored(_, _)));
        } else {
            panic!("Expected Array node");
        }
    }
    ...
    fn test_nested_sequence() {
        ...
        if let Node::Array(items) = result {
            assert_eq!(items.len(), 2);
            assert!(matches!(items[0], Node::Str(_, _, _)));
            assert!(matches!(items[1], Node::Array(ref v) if v.len() == 2));
        } else {
            panic!("Expected Array node");
        }
    }
    ...
    fn test_invalid_indentation_error() {
        ...
        let ctx = ParsingContext::default();
        let mut stream = TokenStream::new(&mut source, &directives, false).unwrap();
        let result = parse_sequence_with_tokens(&mut stream, 0, 0, &directives, &ctx, 0);
