...
fn demo_capacity_hints() {
    ...
    println!("  Before: mapping={}, sequence={}", 
        adaptive.mapping_pairs, adaptive.sequence_items);
    adaptive.update(20, 15);
    println!("  After update(20, 15): mapping={}, sequence={}", 
        adaptive.mapping_pairs, adaptive.sequence_items);
}
...
fn demo_string_pool() {
    ...
    println!("  'name' references share same memory: {}", 
    ...
    println!("  Unique strings: {} (only 4 allocations for 400 strings!)", pool.len());
}
...
fn demo_fast_path_detection() {
    ...
    for (input, description) in test_cases.iter() {
        ...
        println!("    Simple scalar: {}, Simple int: {}", is_simple_scalar, is_simple_int);
    }
    ...
    println!("  Simple doc can use fast path: {}", 
        FastPathDetector::can_use_fast_path(simple_doc));
    println!("  Complex doc can use fast path: {}", 
